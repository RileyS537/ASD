...
def multi_kernel_no_lstm(sample_pic_length, eye_movement, nclasses):  # 350, 900->300, 4
    ...
    flat = Flatten()(conv_dropout2)
    fc1 = Dense(80, activation="relu")(flat)
    fc1_drop = Dropout(0.7)(fc1)
    fc2 = Dense(nclasses, activation="softmax")(fc1_drop)

    model = Model(inputs=inputs, outputs=fc2)
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['acc'])
    plot_model(model, to_file='multi_kernel_no_lstm.png')
    print(model.summary())
    return model

def multi_kernel_no_lstm_gpu1(sample_pic_length, eye_movement, nclasses, lstm_output=False):
    inputs = Input(shape=(sample_pic_length, eye_movement, 4))        # 350 900->300, 4
...
def multi_kernel_no_lstm_gpu2(sample_pic_length, eye_movement, nclasses, lstm_output=False):
    inputs = Input(shape=(sample_pic_length, eye_movement, 4))        # 350 900->300, 4

    x1 = Conv2D(32, (1, 10), padding='same', kernel_regularizer=regularizers.l1(0.), activation='relu')(inputs)
    x1_dropout = Dropout(0.7)(x1)
    average_pool1 = AveragePooling2D((1, 10), strides=(1, 5))(x1_dropout)
    x2 = Conv2D(32, (1, 20), padding='same', kernel_regularizer=regularizers.l1(0.), activation='relu')(inputs)
    x2_dropout = Dropout(0.7)(x2)
    average_pool2 = AveragePooling2D((1, 10), strides=(1, 5))(x2_dropout)
    x3 = Conv2D(32, (1, 30), padding='same', kernel_regularizer=regularizers.l1(0.), activation='relu')(inputs)
    x3_dropout = Dropout(0.7)(x3)
    average_pool3 = AveragePooling2D((1, 10), strides=(1, 5))(x3_dropout)
    x4 = Conv2D(32, (1, 40), padding='same', kernel_regularizer=regularizers.l1(0.), activation='relu')(inputs)
    x4_dropout = Dropout(0.7)(x4)
    average_pool4 = AveragePooling2D((1, 10), strides=(1, 5))(x4_dropout)
